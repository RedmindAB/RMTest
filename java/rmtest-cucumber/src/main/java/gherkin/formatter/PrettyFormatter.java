...
public class PrettyFormatter implements Reporter, Formatter {
    ...
    private final List<MatchResultPair> matchesAndResults = new ArrayList<>();
    ...
    public void row(List<CellResult> cellResults) {
        ...
        cellResults.forEach(cellResult -> {
            cellResult.getResults().stream()
                .filter(result -> (result.getErrorMessage() != null && !seenResults.contains(result)))
                .forEach(result -> {
                    printError(result);
    ...
    private void calculateLocationIndentations() {
        ...
        int maxLineWidth = 0;
        for (BasicStatement currentStatement : statements) {
            int stepWidth = currentStatement.getKeyword().length() + currentStatement.getName().length();
